                                          ...
                                          const float mm_per_pixel)
{
    ...
    const float row_center = location(1) / mm_per_pixel;
    const float col_center = location(0) / mm_per_pixel;
...
void draw_footer(cv::Mat1b &image, const BoardHexGrid &board, const float mm_per_pixel)
{
...
void draw_footer(cv::Mat1b &image, const BoardCircleGrid &board, const float mm_per_pixel)
{
    ...
    cv::putText(image, footer,
                cv::Point(int(0.6 * (board.top_left_(0) - board.outer_radius_) / mm_per_pixel),
                          int(0.6f * (board.top_left_(1) - board.outer_radius_) / mm_per_pixel)),
                cv::FONT_HERSHEY_COMPLEX, footer_scale, cv::Scalar(200, 200, 200), footer_thickness, cv::LINE_AA,
                false);
}
                                               ...
                                               const float radius_in_pixel, const float in_value, const float out_value)
{
    ...
    const float norm = float(point_to_center.norm());
    ...
    if (std::abs(diff) > std::sqrt(0.5f * 0.5f * 2.f))
    {
        if (diff < 0.f)
        {
...
cv::Mat1b board::draw_canonical_board(const BoardRectGrid &board, const float mm_per_pixel)
{
    const Eigen::Vector3d dimension = board.bottom_right_;

    const int rows = std::ceil(dimension(1) / mm_per_pixel + 1);
    const int cols = std::ceil(dimension(0) / mm_per_pixel + 1);
...
cv::Mat1b board::draw_canonical_board(const BoardHexGrid &board, const float mm_per_pixel,
